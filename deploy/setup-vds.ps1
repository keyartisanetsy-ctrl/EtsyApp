...
if ($PublicIp) {
  ...
  Write-Host ""
  Write-Host " This address never changes -- safe to register with Shopify, Etsy, or"
  Write-Host " anywhere else that needs one, and forget it, for as long as this VDS"
  Write-Host " keeps this IP. If the IP ever does change (a new VDS, a provider"
  Write-Host " reassignment), re-run this script to pick up the new one, and update"
  Write-Host " the callback URL registered with each provider to match."
